  ...
  up: async (queryInterface, Sequelize) => {
    try {
      ...
      createdUsers.forEach((user) => {
        let nbArticles = (Math.ceil(Math.random()*10)%8) + 2;
        let Day = new Date(user.createdAt);
      ...
      for (let i = 0; i < 10; i++) {
        tags.push({
          name: `${faker.commerce.product()} ${faker.commerce.productAdjective()} ${faker.company.bsNoun()}`,
          createdAt: new Date(),
          updatedAt: new Date(),
        });
      }
      ...
      createdArticles.forEach((article) => {
        ...
        let nbArticlesTags = (Math.ceil(Math.random() * 10)%6)+2;
