    ...
    .then((data) => {
      data.map((user) => {
        ...
        <tr>
            ...
            <td>${user.role}</td>
            <td class="text-right">
              <a class="btn btn-outline-primary" onclick="updateUser(event, ${user.id}, '${user.username}', '${user.email}')">Edit</a>
              <a class="btn btn-outline-danger" onclick="deleteUser(event, ${user.id})">Delete</a>
            </td>
        </tr>`;
      });
    });
